...
**第一步：采用冻结源单元（source.units）**
- 若用户提供【冻结源单元】，它是程序在调用模型前确定的审查范围。必须逐项、按原顺序复制其 `id`、`text`、`para`，不得合并、拆分、重编号或改写；模型只补 `kind`，以及极少数情况下的 `skipped` / `skip_reason`
- 仅兼容模式未提供【冻结源单元】时，才按句末标点（。！？…以及引号后的句末）切分并顺序编号 u0001、u0002…
...
- 原则上不使用 `skipped`；只有纯元叙述（如"欲知后事如何"）才可标 `skipped: true`，并必须填写具体、可人工复核的 `skip_reason`
...
**第四步：自检后输出**
输出前逐项核对：冻结 unit 的 id/text/para 未被改动；所有 unit 都被 narration/台词/画面覆盖或有理由地 skipped；所有 ref 指向存在的资产；所有 inferred 镜头有 inference_note；所有枚举值来自受控词表。`coverage` 可填写，但最终值由程序独立重算，模型声明不作为验收依据。
