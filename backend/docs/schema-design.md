# 分镜 JSON Schema 设计说明（v0.5 · 忠实转换模式）
...
由一条业内优秀的长镜头视频提示词逆推出的两处表达缺口。**两个字段皆可选，v0.4 的产物一个字不改照常合法**（`schema_version` 由 `const: "0.4"` 放宽为 `enum: ["0.4", "0.5"]`）：

- **镜头内阶段 `shot.beats[]`**：一个不切的长镜头里，情绪/灯光/表演可以逐级推进（如：强忍→落泪→失笑→拍桌爆发→烛光收束，全程一镜到底只换光）。此前 `action`/`atmosphere` 各只有一份，这种推进只能塞进一根字符串任下游去猜——而 `prompts.video_prompt` 的描述里本就写着"时间轴格式"，说明设计时想到了、结构上没给承载。每拍 `duration_sec`(必填) / `action`(必填) / `expression` / `lighting` / `camera_note`；`minItems: 2`（只分一拍等于没分）
- **负面约束 `shot.constraints[]`**：镜头级"明确不要出现什么"（画面内不出现任何灯具、无镜头光晕、全程不切镜）。`meta.style.negative_prompt` 是全局的，压不住单镜头的具体禁令，而出图质量的一半毁在没有约束上
- lint 新增：各拍时长之和必须等于镜头 `duration_sec`（容差 0.05s，两者同时存在却对不上时下游按哪个走全凭猜）、空 action 的拍与空约束项报错
- **生成器暂不产出这两个字段**，定位为人工在编辑器里补：133 个镜头逐一写到这种颗粒度，token 会爆且弱模型质量会崩。现实用法是生成器给骨架、人给高潮戏加拍子。前端编辑器一旦写入这两个字段，会自动把 `meta.schema_version` 顶到 `"0.5"`
- 灯光维度**未做结构化**（方向/强度/色温/运动），先由 `beats[].lighting` 自由文本承载——等真跑几次出图看效果再决定是否值得建模

第二批（由 Higgsfield《Kök Börü》的角色设定表编辑提示词逆推）。对方那条提示词有一半篇幅在写"什么必须不变"（脸/发/体型/三个姿势/背景/笔触/分辨率），只改服装一项——与本 IR"形象靠引用锁死、描述只写增量"是同一个信念，缺的是设定图这一环：

- **设定图归属变体**：`reference_images[]` 条目新增 `outfit`（角色卡）/ `state`（道具卡）标注。设定图是"画师修正稿反过来约束后续每一帧"这条链的锚点，而它实际属于**某一套服装**而非整个角色——对方那条提示词产出的正是"暖季装的三视图"。不标注就会拿另一套衣服的脸去垫图，等于白垫。lint 新增交叉校验：标注的 outfit/state 必须存在于本卡
- **`view` 枚举补 `close_up`**：对方的设定表是"全身正面 + 全身四分之三侧 + 面部特写"，原枚举 front/side/back/three_quarter 表达不了特写。道具与生物卡的 `reference_images` 一并对齐为同一形状（原先只有 `uri`）
- **资产卡级负面约束**：`assets.*[].constraints`，与 `shot.constraints` 共用 `$defs/constraints`。卡上是"画这张卡时不要做什么"（不要平滑笔触/不要加照片级细节/不要改画风），`meta.style.negative_prompt` 是全局的，压不住单张卡的具体禁令
- **生成器仍不产出设定图**：`uri` 由人工或外部出图工具填入，IR 只记录引用、不负责生成——这条链接得上，产品边界不动
- 顺带修正了前端 TS 镜像与 schema 的三处错配（`outfits[]`/`states[]` 实为 `{id, description}`、`locations[].angles[]` 实为 `{angle, uri}`），此前编辑器按错误形状写字段，改一次服装描述整份产物就 schema 非法

## 后续版本方向（v0.6+）

- ~~剪映草稿 JSON 适配器~~ 移出范围：产品边界收在 IR 本身，下游消费方式不做承诺
- 结构化灯光与 `camera.texture`（"固定机位但保留手持呼吸感"这类叠加描述现无处安放）——待 v0.5 的自由文本用出经验再定
- 建议由下一轮真实网文（句式混乱度更高的连载文）实测驱动
